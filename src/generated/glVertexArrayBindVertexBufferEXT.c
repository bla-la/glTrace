...
GLAPI void  APIENTRY glVertexArrayBindVertexBufferEXT(GLuint vaobj,GLuint bindingindex,GLuint buffer,GLintptr offset,GLsizei stride)
{
	...
	if(!GL_ENTRY_PTR(glVertexArrayBindVertexBufferEXT_Idx))
	{
            GL_ENTRY_PTR(glVertexArrayBindVertexBufferEXT_Idx) = dlsym(RTLD_NEXT,"glVertexArrayBindVertexBufferEXT");
            if(!GL_ENTRY_PTR(glVertexArrayBindVertexBufferEXT_Idx))
                abort();
	}
