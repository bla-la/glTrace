...
GLAPI void  APIENTRY glRenderbufferStorageMultisampleANGLE(GLenum target,GLsizei samples,GLenum internalformat,GLsizei width,GLsizei height)
{
	...
	if(!GL_ENTRY_PTR(glRenderbufferStorageMultisampleANGLE_Idx))
	{
            GL_ENTRY_PTR(glRenderbufferStorageMultisampleANGLE_Idx) = dlsym(RTLD_NEXT,"glRenderbufferStorageMultisampleANGLE");
            if(!GL_ENTRY_PTR(glRenderbufferStorageMultisampleANGLE_Idx))
                abort();
	}
