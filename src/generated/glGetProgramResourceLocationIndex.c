...
GLAPI GLint  APIENTRY glGetProgramResourceLocationIndex(GLuint program,GLenum programInterface,const GLchar *name)
{
	...
	if(!GL_ENTRY_PTR(glGetProgramResourceLocationIndex_Idx))
	{
            GL_ENTRY_PTR(glGetProgramResourceLocationIndex_Idx) = dlsym(RTLD_NEXT,"glGetProgramResourceLocationIndex");
            if(!GL_ENTRY_PTR(glGetProgramResourceLocationIndex_Idx))
                abort();
	}
