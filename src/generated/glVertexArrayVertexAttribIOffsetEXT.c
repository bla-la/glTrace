...
GLAPI void  APIENTRY glVertexArrayVertexAttribIOffsetEXT(GLuint vaobj,GLuint buffer,GLuint index,GLint size,GLenum type,GLsizei stride,GLintptr offset)
{
	...
	if(!GL_ENTRY_PTR(glVertexArrayVertexAttribIOffsetEXT_Idx))
	{
            GL_ENTRY_PTR(glVertexArrayVertexAttribIOffsetEXT_Idx) = dlsym(RTLD_NEXT,"glVertexArrayVertexAttribIOffsetEXT");
            if(!GL_ENTRY_PTR(glVertexArrayVertexAttribIOffsetEXT_Idx))
                abort();
	}
