...
GLAPI void  APIENTRY glNamedProgramLocalParameterI4uivEXT(GLuint program,GLenum target,GLuint index,const GLuint *params)
{
	...
	if(!GL_ENTRY_PTR(glNamedProgramLocalParameterI4uivEXT_Idx))
	{
            GL_ENTRY_PTR(glNamedProgramLocalParameterI4uivEXT_Idx) = dlsym(RTLD_NEXT,"glNamedProgramLocalParameterI4uivEXT");
            if(!GL_ENTRY_PTR(glNamedProgramLocalParameterI4uivEXT_Idx))
                abort();
	}
