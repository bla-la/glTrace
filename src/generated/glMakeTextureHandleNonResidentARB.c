...
GLAPI void  APIENTRY glMakeTextureHandleNonResidentARB(GLuint64 handle)
{
	...
	if(!GL_ENTRY_PTR(glMakeTextureHandleNonResidentARB_Idx))
	{
            GL_ENTRY_PTR(glMakeTextureHandleNonResidentARB_Idx) = dlsym(RTLD_NEXT,"glMakeTextureHandleNonResidentARB");
            if(!GL_ENTRY_PTR(glMakeTextureHandleNonResidentARB_Idx))
                abort();
	}
