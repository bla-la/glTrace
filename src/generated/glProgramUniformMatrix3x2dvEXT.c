...
GLAPI void  APIENTRY glProgramUniformMatrix3x2dvEXT(GLuint program,GLint location,GLsizei count,GLboolean transpose,const GLdouble *value)
{
	...
	if( !GL_ENTRY_PREV_TS(glProgramUniformMatrix3x2dvEXT_Idx))
    	{
            GL_ENTRY_PREV_TS(glProgramUniformMatrix3x2dvEXT_Idx) = get_ts();
        }

        clock_gettime(CLOCK_REALTIME,&st);

	glProgramUniformMatrix3x2dvEXT_wrp(program,location,count,transpose,value);

        clock_gettime(CLOCK_REALTIME,&ed);

        GL_ENTRY_CALL_TIME(glProgramUniformMatrix3x2dvEXT_Idx) += ts_diff(&st,&ed);
        ...
        if(last_diff > 1000000000){
            printf("glProgramUniformMatrix3x2dvEXT %lld %lld avg %lld  total time left %lld pct %f\n",
	             GL_ENTRY_CALL_COUNT(glProgramUniformMatrix3x2dvEXT_Idx),
	             GL_ENTRY_CALL_TIME(glProgramUniformMatrix3x2dvEXT_Idx),
	             GL_ENTRY_CALL_TIME(glProgramUniformMatrix3x2dvEXT_Idx) /
	             GL_ENTRY_CALL_COUNT(glProgramUniformMatrix3x2dvEXT_Idx),
	             last_diff,GL_ENTRY_CALL_TIME(glProgramUniformMatrix3x2dvEXT_Idx)/ (last_diff/100.0));
             GL_ENTRY_CALL_COUNT(glProgramUniformMatrix3x2dvEXT_Idx) = 0;
             GL_ENTRY_CALL_TIME(glProgramUniformMatrix3x2dvEXT_Idx)  = 0;
	     GL_ENTRY_PREV_TS(glProgramUniformMatrix3x2dvEXT_Idx) = get_ts();
        }
