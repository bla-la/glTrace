...
GLAPI void  APIENTRY glMultiDrawElementsIndirectBindlessNV(GLenum mode,GLenum type,const void *indirect,GLsizei drawCount,GLsizei stride,GLint vertexBufferCount)
{
	...
	if(!GL_ENTRY_PTR(glMultiDrawElementsIndirectBindlessNV_Idx))
	{
            GL_ENTRY_PTR(glMultiDrawElementsIndirectBindlessNV_Idx) = dlsym(RTLD_NEXT,"glMultiDrawElementsIndirectBindlessNV");
            if(!GL_ENTRY_PTR(glMultiDrawElementsIndirectBindlessNV_Idx))
                abort();
	}
