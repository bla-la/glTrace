...
GLAPI void  APIENTRY glMultiDrawElementsIndirectCountARB(GLenum mode,GLenum type,GLintptr indirect,GLintptr drawcount,GLsizei maxdrawcount,GLsizei stride)
{
	...
	if(!GL_ENTRY_PTR(glMultiDrawElementsIndirectCountARB_Idx))
	{
            GL_ENTRY_PTR(glMultiDrawElementsIndirectCountARB_Idx) = dlsym(RTLD_NEXT,"glMultiDrawElementsIndirectCountARB");
            if(!GL_ENTRY_PTR(glMultiDrawElementsIndirectCountARB_Idx))
                abort();
	}
