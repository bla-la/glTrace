...
GLAPI void  APIENTRY glGetTranslatedShaderSourceANGLE(GLuint shader,GLsizei bufsize,GLsizei *length,GLchar *source)
{
	...
	if(!GL_ENTRY_PTR(glGetTranslatedShaderSourceANGLE_Idx))
	{
            GL_ENTRY_PTR(glGetTranslatedShaderSourceANGLE_Idx) = dlsym(RTLD_NEXT,"glGetTranslatedShaderSourceANGLE");
            if(!GL_ENTRY_PTR(glGetTranslatedShaderSourceANGLE_Idx))
                abort();
	}
