...
GLAPI void  APIENTRY glBindVideoCaptureStreamTextureNV(GLuint video_capture_slot,GLuint stream,GLenum frame_region,GLenum target,GLuint texture)
{
	...
	if(!GL_ENTRY_PTR(glBindVideoCaptureStreamTextureNV_Idx))
	{
            GL_ENTRY_PTR(glBindVideoCaptureStreamTextureNV_Idx) = dlsym(RTLD_NEXT,"glBindVideoCaptureStreamTextureNV");
            if(!GL_ENTRY_PTR(glBindVideoCaptureStreamTextureNV_Idx))
                abort();
	}
