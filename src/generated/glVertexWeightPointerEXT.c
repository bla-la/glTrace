...
#define glVertexWeightPointerEXT_wrp						\
    ((void  (*)(GLint size,GLenum type,GLsizei stride,const void *pointer                                        \
    ))GL_ENTRY_PTR(glVertexWeightPointerEXT_Idx))
...
//init on start
	...
	if( !GL_ENTRY_PREV_TS(glVertexWeightPointerEXT_Idx))
    	{
            GL_ENTRY_PREV_TS(glVertexWeightPointerEXT_Idx) = get_ts();
        }

        clock_gettime(CLOCK_REALTIME,&st);

	glVertexWeightPointerEXT_wrp(size,type,stride,pointer);

        clock_gettime(CLOCK_REALTIME,&ed);

        GL_ENTRY_CALL_TIME(glVertexWeightPointerEXT_Idx) += ts_diff(&st,&ed);
        ...
        if(last_diff > 1000000000){
            printf("glVertexWeightPointerEXT %lld %lld avg %lld  total time left %lld pct %f\n",
	             GL_ENTRY_CALL_COUNT(glVertexWeightPointerEXT_Idx),
	             GL_ENTRY_CALL_TIME(glVertexWeightPointerEXT_Idx),
	             GL_ENTRY_CALL_TIME(glVertexWeightPointerEXT_Idx) /
	             GL_ENTRY_CALL_COUNT(glVertexWeightPointerEXT_Idx),
	             last_diff,GL_ENTRY_CALL_TIME(glVertexWeightPointerEXT_Idx)/ (last_diff/100.0));
             GL_ENTRY_CALL_COUNT(glVertexWeightPointerEXT_Idx) = 0;
             GL_ENTRY_CALL_TIME(glVertexWeightPointerEXT_Idx)  = 0;
	     GL_ENTRY_PREV_TS(glVertexWeightPointerEXT_Idx) = get_ts();
        }
