...
GLAPI void  APIENTRY glVertexArraySecondaryColorOffsetEXT(GLuint vaobj,GLuint buffer,GLint size,GLenum type,GLsizei stride,GLintptr offset)
{
	...
	if(!GL_ENTRY_PTR(glVertexArraySecondaryColorOffsetEXT_Idx))
	{
            GL_ENTRY_PTR(glVertexArraySecondaryColorOffsetEXT_Idx) = dlsym(RTLD_NEXT,"glVertexArraySecondaryColorOffsetEXT");
            if(!GL_ENTRY_PTR(glVertexArraySecondaryColorOffsetEXT_Idx))
                abort();
	}
