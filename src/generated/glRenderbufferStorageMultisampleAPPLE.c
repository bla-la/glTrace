...
GLAPI void  APIENTRY glRenderbufferStorageMultisampleAPPLE(GLenum target,GLsizei samples,GLenum internalformat,GLsizei width,GLsizei height)
{
	...
	if(!GL_ENTRY_PTR(glRenderbufferStorageMultisampleAPPLE_Idx))
	{
            GL_ENTRY_PTR(glRenderbufferStorageMultisampleAPPLE_Idx) = dlsym(RTLD_NEXT,"glRenderbufferStorageMultisampleAPPLE");
            if(!GL_ENTRY_PTR(glRenderbufferStorageMultisampleAPPLE_Idx))
                abort();
	}
